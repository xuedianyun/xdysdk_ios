...
@property (strong, nonatomic)  UIButton       *docicon;
...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _docicon = [UIButton buttonWithType:(UIButtonTypeCustom)];
        [self addSubview:_docicon];
...
- (void)layoutSubviews{
    ...
    self.docicon.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
//    [self.docicon setImage:[self setImageOriginal:@"XueDianYunSDK.bundle/doc.png"] forState:(UIControlStateNormal)];
    self.docicon.backgroundColor = [UIColor clearColor];
    _scrollView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    _displayView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    _drawView.frame = self.displayView.bounds;
