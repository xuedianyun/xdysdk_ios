...
- (void)showFromleftOffset:(CGFloat)left topOffset:(CGFloat)top{
    UIWindow *window = [[[UIApplication sharedApplication] windows] lastObject];
    _contentView.center = CGPointMake(left+_contentView.frame.size.width/2,top + _contentView.frame.size.height/2);
    [window  addSubview:_contentView];
    [self showAnimation];
    [self performSelector:@selector(hideAnimation) withObject:nil afterDelay:_duration];
}
...
#pragma mark-中间显示
+ (void)showCenterWithText:(NSString *)text  leftOffset:(CGFloat)leftOffset topOffset:(CGFloat)topOffset{
    [XDYAlert showCenterWithText:text duration:ToastDispalyDuration leftOffset:leftOffset topOffset:topOffset];
}

+ (void)showCenterWithText:(NSString *)text duration:(CGFloat)duration leftOffset:(CGFloat)leftOffset topOffset:(CGFloat)topOffset{
    XDYAlert *toast = [[XDYAlert alloc] initWithText:text];
    [toast setDuration:duration];
    [toast showFromleftOffset:leftOffset topOffset:topOffset];
//    [toast show];
}
...
+ (void)showTopWithText:(NSString *)text{
    ...
    [XDYAlert showTopWithText:text  topOffset:ToastSpace duration:ToastDispalyDuration];
}
+ (void)showTopWithText:(NSString *)text duration:(CGFloat)duration
{
     [XDYAlert showTopWithText:text  topOffset:ToastSpace duration:duration];
}
+ (void)showTopWithText:(NSString *)text topOffset:(CGFloat)topOffset{
    [XDYAlert showTopWithText:text  topOffset:topOffset duration:ToastDispalyDuration];
}
...
+ (void)showBottomWithText:(NSString *)text{
    ...
    [XDYAlert showBottomWithText:text  bottomOffset:ToastSpace duration:ToastDispalyDuration];
}
+ (void)showBottomWithText:(NSString *)text duration:(CGFloat)duration
{
      [XDYAlert showBottomWithText:text  bottomOffset:ToastSpace duration:duration];
}
+ (void)showBottomWithText:(NSString *)text bottomOffset:(CGFloat)bottomOffset{
    [XDYAlert showBottomWithText:text  bottomOffset:bottomOffset duration:ToastDispalyDuration];
}
