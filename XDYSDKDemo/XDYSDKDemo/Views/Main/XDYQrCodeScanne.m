...
#define scanneArea 240
#define scanneAreaIpad 500

#define SYScanne_Line_Tag 57976

#define SYCenter_Tag 238374

@interface XDYQrCodeScanne () <AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation XDYQrCodeScanne
...
- (void)createUI
{
    ...
    if (isPad) {
        scanArea = scanneAreaIpad;
    }else{
        scanArea = scanneArea;
    }
    
    //上下两部分高度
    CGFloat height = (SYScreenHigh-scanArea)/2;
    
    //左右两部分宽度
    CGFloat width = (SYScreenWidth-scanArea)/2;
    ...
    //左部蒙版
    UIView *leftView = [[UIView alloc]initWithFrame:CGRectMake(0, height, width, scanArea)];
    leftView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.7];
    [self.view addSubview:leftView];
    
    //右部蒙版
    UIView *rightView = [[UIView alloc]initWithFrame:CGRectMake(SYScreenWidth-width, height, width, scanArea)];
    ...
    [backBtn setBackgroundImage:[UIImage imageNamed:@"qrback"] forState:UIControlStateNormal];
    [backBtn addTarget:self action:@selector(onBackClick) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:backBtn];
    
    
    //扫描框
    UIImageView *centerView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, scanArea, scanArea)];
    centerView.tag = SYCenter_Tag;
    centerView.center = self.view.center;
    centerView.image = [UIImage imageNamed:@"scanBoard"];
    ...
    //扫描线
    UIImageView *scanneLine = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetMinX(centerView.frame), CGRectGetMaxY(topView.frame), scanArea, 3)];
    scanneLine.tag = SYScanne_Line_Tag;
    [scanneLine setImage:[UIImage imageNamed:@"scanLine"]];
    ...
    [photoBtn setBackgroundImage:[UIImage imageNamed:@"qrphoto"] forState:UIControlStateNormal];
    ...
    [openFlashBtn setBackgroundImage:[UIImage imageNamed:@"qrs.png"] forState:UIControlStateNormal];
    [openFlashBtn setBackgroundImage:[UIImage imageNamed:@"qrSelect"] forState:UIControlStateSelected];
...
- (void)openScanning
{
    ...
    //限制扫描范围
    [output setRectOfInterest:CGRectMake (((SYScreenHigh - scanArea)/2)/SYScreenHigh, ((SYScreenWidth - scanArea)/2)/SYScreenWidth, scanArea/SYScreenHigh, scanArea/SYScreenWidth)];
